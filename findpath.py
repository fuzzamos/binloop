...
print filename
print "These functions call {}".format(targetFunction)

allPaths = []
def whatCalls(fn, chain = [], indent = 1):
    global allPaths
    if len(list(set(chain))) < len(chain):
        return
    allPaths += [ deepcopy(chain) ]
    for k, v in calls.iteritems():
        found = False
        for _v in v:
            #if _v.endswith(targetFunction):
            if _v == fn:
                found = True
                break
        if found == False:
            continue
        #print (indent*" ") + "|->" + k
        whatCalls(k, chain + [k], indent+1)
whatCalls(targetFunction)
